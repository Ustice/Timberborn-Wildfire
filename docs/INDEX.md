...
- [Natural-water ownership and QA source creation](qa/native-water-source-factory.md): native volume remains authoritative, with persisted source identity and exact scheduled credit observation. Earlier CPU Unity probes verify their recorded factory, removal, restore and map boundaries; the new normal Game source/shore composition still needs colony validation.
- [Native source-to-bucket conversion](qa/warden-natural-source-fill.md): exact native clean credit becomes Water through Produced accounting, after credit settles; positive public pickup and complete physical delivery remain open.
- [Native water removal and precision](qa/native-water-removal-task.md): actual finite-column removal produces receipts; full-quantum retry fills a bucket and preserves native surplus without rounding up insufficient volume.
...
- [Warden water transfer callbacks](qa/warden-water-transfer-callbacks.md): native return rejects a destination disabled after debit; existing failure guards block saving incomplete transfers.
- [Reserved water return](qa/warden-reserved-return.md): actual Unity components prove one reserved Water return and disabled-destination refusal, including native reservation release; full adult lifecycle and physical recovery remain open.
- [Borrowed-water recovery](qa/borrowed-water-return-recovery.md): persistent return assignment uses native arbitration and reserves capacity only at arrival; physical recovery and complete actor restore remain open.
- [Connected finite Folktails sortie](qa/folktails-finite-water-sortie.md): development commands connect source creation, borrowed work, settled pickup, shared suppression and cancellation recovery; shipping Fire Bell controls and actual complete trips remain open.
- [Native QA session controls](qa/native-session-control.md): guarded native save copies with closed-file receipts and pause/resume requests; complete game-save and frame application remain live checks.
...
- [Current-world growth readiness](qa/ash-growth-current-world-readiness.md): current transport, global enablement, settled native reads and callback-safe cache publication are source-tested; production binding remains open.
- [Initialized ash-growth component proof](qa/ash-growth-initialized-component.md): actual CPU Unity entity/timer/soil-base application and callback-failure guard; authored requests, no production readiness or live fertility claim.
...
- [Inactive finite fertilizer job](qa/finite-fertilizer-job.md): native job ownership, guarded acceptance/rejection, cancel-on-load intent and explicit pending-offer cancellation; Claude review completed; supplied Ready-through-Runtime now passes, physical tree travel remains open.
...
- [Finite fertilizer Runtime/GPU proof](qa/finite-fertilizer-runtime-gpu.md): actual native pickup, concrete supplied Ready executor, Metal acceptance/rejection, native consumption and separate current-transport late growth; no physical trip or shipping binding.
