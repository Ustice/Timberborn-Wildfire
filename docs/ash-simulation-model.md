...
Tainted ash is not permanent by default. It should fade over time, and water contact can wash it away. Washed tainted ash should slightly taint the affected water through a bounded adapter mutation when a Timberborn API exists; otherwise the washout should still be visible in ash state and report water-taint skip telemetry. Clean ash washing does not taint water, and badwater or contaminated water touching ash does not become clean.
