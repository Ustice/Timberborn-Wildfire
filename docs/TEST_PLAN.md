...
- Timberborn GPU field renderer: compact-delta visual events now feed a primary region-batched renderer that samples the GPU visual-field surface, aggregates fire, smoke, ash, derived steam, visibility, and heat-haze intensities by bounded regions, and presents the result through a single Unity mesh surface instead of one object per burning cell. `status` and `qa-readiness` expose renderer enabled/material/surface state plus visible, updated, dropped, and failed region counters. `TWF-155` tracks a follow-up split between true dropped regions and below-threshold invisible regions.
