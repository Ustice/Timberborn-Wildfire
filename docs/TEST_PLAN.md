...
- Ash: older snapshot and visual validation may treat ash as derived visual output, but gameplay ash must be validated through simulator transport state. Passing evidence for release screenshots can still show temporary visual ash; gameplay consequences must not rely on renderer-owned ash.
