...
- `TWF-041` and `TWF-042` are ready for the remaining player-legibility sprint work: visual tuning with evidence and one player-facing fire alert.
...
- Continue the player-legibility sprint with `TWF-041`: tune fire, smoke, ash, and visibility output using shader snapshots and live Timberborn visual evidence now that pooled native presentation is integrated.
...
- The current sprint milestone is player legibility: the GPU visual-field surface and bounded pooled native presentation are integrated, but visual tuning evidence and one player-facing fire alert/status surface still remain.
