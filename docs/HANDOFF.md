...
- `TWF-065` through `TWF-069` are todo for the next tuning phase: add high-resolution and low-resolution screen recording tooling, then tune fire, smoke, ash, and fire behavior as separate evidence loops.
...
- Do not treat visual tuning as one ticket: `TWF-066`, `TWF-067`, `TWF-068`, and `TWF-069` intentionally split fire effect, smoke effect, ash effect, and fire behavior tuning so each can use the right recording evidence.
