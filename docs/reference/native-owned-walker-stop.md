...
WardenExecutor is an IExecutor, not IPersistentEntity: BehaviorManager invokes its Load only when resolving it as the saved running executor. Nevertheless an Idle payload must leave movement untouched. Load only reads active payload and marks replan. The first active executor tick verifies BehaviorManager.IsRunningExecutor<WardenExecutor>() before any stop, then stops and replans, because native Walker.Load/NavMeshObserver.PostLoad can restore a destination after the executor's Load call. Waiting for a paused mover to reach that destination is not a valid restore protocol. Saved active route restoration still requires copied-save gameplay proof.
