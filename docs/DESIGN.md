...
The GPU output reserves capacity for both external-change and simulation records. A cell can appear more than once in a tick. A `CellDelta` contains an index, old/new packed cell values, and the originating material target id. Owned consequences resolve that retained identity rather than whichever entity currently occupies the cell. It is not a complete transport snapshot. Consumers needing ash, smoke, or steam read the corresponding simulation fields. Renderers may smooth or aggregate these fields for presentation without authoring gameplay state.
