...
Water reaching settled ash washes that ash out through the simulator ash mutation path, so the next simulator state owns the removal and persistence keeps any remaining ash. The shader simulation step preserves ash under water until a queued ash mutation is applied; this keeps washout visible through `ash_water_washout_*`, `water_taint_attempts`, and `skipped_unsafe_water_apis` telemetry instead of silently deleting ash. Clean water washing clean ash removes ash only; it must not create tainted water. Clean water washing tainted ash attempts a small bounded water-taint mutation when Timberborn exposes a safe API, and otherwise reports safe-unavailable water-taint telemetry while still washing the ash out. Badwater or already contaminated water may wash ash away, but it must never become clean because it touched ash.
