...
`TimberbornOwnedStorageDeltaConsumer` shares `TimberbornOwnedBurnOrigins` with trees: whole-batch TargetId→retained Guid→canonical family/key preflight precedes effects. The owned damage reducer sums normalized fuel-loss contributions and writes each owner once. The storage sink processes each accepted contribution in stable spatial order, with hazards at the cell that completes the item cost; see [transition normalization](owned-transition-normalization.md). The separate storage sink rechecks each native target and consumes each planned stack before any hazard effect. Hazard amounts come exclusively from successful actual removal receipts; the original delta cell remains the intentional blast/contamination location. Earlier callbacks can delete a later target; its replacement remains untouched.
