...
Actual installed singleton loading orders ordinary tickers before `ILateTickable`; the clock and native TimeTriggerService are ordinary. A native resource already grown, paused, dead, or dying receives no bonus, and the adapter does not force resume. Exact owner identity, initialization/Unity liveness, current cell membership, and unchanged duration are rechecked before application. Preparation also requires the request XYZ to equal native `BlockObject.CoordinatesAtBaseZ` and resolve through native `MapIndexService.CellToIndex` / `IThreadSafeColumnTerrainMap.TryGetIndexAtCeiling`. Application rechecks that exact base and the original column index; movement or changed spatial eligibility skips the candidate. Candidates deduplicate by exact object reference, preserving the previous first-reference request behavior; no maximum-across-footprint policy was introduced.
...
- Actual native BlockService/Array3D methods resolve a tall resource at both z=1 and z=2 and a separate upper resource at z=3. The original upper-cell regression failed (expected 0 candidates, actual 1); the native base/ceiling filter passes. Separate cases require both base equality and a real column ceiling. Native HasGrown callbacks move a base, change its ceiling, or compact a valid upper column to a different index; the prepared spatial predicate rejects each change. Owners remain deliberately uninitialized: these managed tests exercise the spatial predicate directly and do not claim a positive Unity application path.
...
The [initialized CPU component probe](ash-growth-initialized-component.md) now establishes actual Entity.Initialize, native BlockService.SetObject and the positive prepared application/callback-failure path on448a1be. Its clean requests are authored fixture inputs; the production readiness/GPU acceptance and full world gates below remain open.

Do not bind the ticker merely because the arithmetic and exact block lookup pass. The eventual activation proof must combine a real planted GameObject/component graph, native BlockService registration, actual native column ceiling c, and the current-world readiness path. It must show that the simulator-accepted clean-ash cell is the actual soil-ceiling cell c, that c−1/c+1 and a stacked upper plant do not gain a soil benefit, and that a tall resource's upper occupied cells cannot qualify merely because they resolve to the same Growable. The prepared adapter now independently checks native base/ceiling identity. Current-world transport readiness, accepted GPU input-to-soil mapping, and the complete live application path remain binding gates. The next supplied CPU/native component fixture can establish the positive adapter path separately; it must label authored clean requests rather than claim a production readiness producer.
...
## Native soil-anchor follow-up

The filter follows installed DryObject's native `CoordinatesAtBaseZ` soil query, with no guessed z offset or nearby-XY widening. A separate upper plant with its own actual column ceiling remains eligible for its own request; an upper occupied cell of the lower plant does not. No native column data is copied or scanned across the world. The adapter takes existing BlockService, MapIndexService, and thread-safe column terrain map dependencies; the ticker remains unbound.

Validation on base `c158d61`: **28 focused growth cases and 1,565 full native tests passed**, zero failures/skips. The focused rebuild reports the two existing nullable warnings in the beaver worker-speed adapter; this slice does not change that file. Evidence: `/tmp/wildfire-growth-soil-anchor/upper-cell-red.log`, `focused.log`, and `full-native.log`. No engine/game/deployment was run. The controller will test actual EntityComponent initialization and native column/BlockService registration against the immutable source pin.

A subsequent unbound [current-world readiness producer](ash-growth-current-world-readiness.md) now derives copied requests from the Runtime field cache with global-enable and native settled-window checks. This replaces the missing source seam; normal Runtime accepted-application visibility and binding still require their separate controller proof.

The subsequent [Stage C Runtime/Metal component proof](finite-fertilizer-runtime-gpu.md) closes accepted clean input at the actual Carrot soil ceiling through current Runtime transport/readiness into a separate settled native interval, with a Tainted/no-extra-growth control and measured ordinary clock progress. Its explicit session/adult/Ready setup does not establish a physical trip, full-world lifecycle, restore behavior or shipping ticker binding.
