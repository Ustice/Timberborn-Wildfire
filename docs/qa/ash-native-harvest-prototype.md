...
Return uses the native capacity reservation as the destination authority. It rechecks current access, actual arrival, safe position, exact reservation quantity, and actual carried type/quantity before `UnreserveCapacity → GiveProduced → EmptyHands → Idle`, all under the shared guard. A removed or unusable destination triggers the original gatherer then native closest district inventory lookup, each with safe-route admission. If the closest choice fails, it tries other currently eligible district inventories rather than treating that one failure as proof that no deposit is possible. No suitable result leaves the worker holding its native cargo, with a visible panel reason. No implicit CarryRoot handoff occurs.
...
- The fallback now tries other active district inventories if the native closest choice cannot be used. It snapshots candidates, orders alternatives by straight-line distance, rechecks current district membership/capacity/native validator/blocking, then uses native reachability plus the existing safe-route and guarded reservation/walk. Full native suite956 passes and installed picker IL confirms these admission contracts; a focused native null-assignment fixture also proves fallback declines before navigation after district assignment is lost (removing the entry guard reproduces a null-reference failure). The actual farther-safe destination scenario and callback-driven reassignment still require live validation. This is not a shortest-safe-path guarantee.
