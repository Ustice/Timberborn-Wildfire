...
The initial snapshot proof emitted `There are more uavs (10) than the maximum supported (8)` for `ApplyExternalChanges`. The later resource-layout correction at `d497c47` makes material requests read-only and stores the header in reserved receipt row zero, removing the separate header UAV. All 34 licensed shader cases (39 captures) and this actual native snapshot/exhaustion probe then pass with zero shader/UAV warnings on Metal Apple M2 Pro. The device reports eight writable targets and 31 compute-buffer inputs. No warning was suppressed. No game world/save publication, native registry activation, or wider hardware support was tested here.
