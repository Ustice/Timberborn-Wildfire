...
### Exit admission follow-up

Installed native IL shows `Mortal.Dead == !Character.Alive`, so ordinary native mortality was already excluded. Clean deletion differs: `EntityComponent.Delete` finishes component callbacks and posts EntityDeletedEvent before `EntityService.Delete` destroys the GameObject. BaseComponent truth checks GameObject truth, not entity deletion. Actual native fixtures observe `_exited=true` while Character remains alive, Mortal.Dead/ShouldDie false, inventory enabled, guard safe and one unit unspent during that later deletion event.

The one-line follow-up puts `!_exited` first in the common stock-admission predicate. Five additional cases cover those native facts and rejection by pickup, return and positively privileged application consumption. For the three short-circuit cases, the test removes only the satchel's supplied cache after its actual native exit callback; the previous source attempts cache access and fails all three. This deliberately verifies ordering without fabricating a live Unity object. With the fix, reservations, native quantities and phase callbacks remain untouched. A positive post-delete call on a retained GameObject remains an engine-only boundary; the native Delete-before-Destroy ordering independently establishes the source defect. Full native validation now passes **1,436 tests**, zero skips.
