...
## Executed component follow-up

The [Stage C Metal proof](finite-fertilizer-runtime-gpu.md) now exercises normal Runtime dispatch/followups, accepted and Tainted GPU receipts, current transport/revision and this actual readiness producer, followed by a separately composed settled native late tick. Clean adds exactly the elapsed bonus; Tainted adds none while ordinary native clock growth still advances. Explicit session publication, supplied adult/Ready/positions and an inert GPU baseline remain fixture inputs. Neither the worker nor late ticker is bound in shipping code; full-world restore, physical trip and remaining failure controls are separate gates.
