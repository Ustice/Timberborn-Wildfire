...
Fresh activation requires a never-seen owner in this bounded prototype. Previously known owners require an explicit available archive or a captured source cell; no fallback to original fuel. This rejects unsupported footprint expansion of existing owners until a native slot registry is implemented. Hidden material pauses with its GPU-authored remaining fuel/raw history. Current shader does not advance BurnHistory; the archive preserves those bits without inventing history from entity damage.
...
A read-only request buffer, writable receipt rows, and a per-cell slot-ID buffer are required, alongside the target-ID buffer. The acknowledged batch marker is always the exclusive final ordinary-input slot. Ordered apply replaces that existing 16-byte slot with the four logical header words: attempt token, accepted/rejected status, request count, and rejected row (or `uint.MaxValue`). Both backends read the actual final uploaded index; an unexecuted marker has token zero in its first word, and a stale receipt cannot match the current single-use attempt token. Ordinary and ash-command layouts are unchanged. This removes the separate writable header buffer and keeps `ApplyExternalChanges` at eight UAVs. Request/receipt buffers start at one row and grow together to the admitted batch size, bounded by grid capacity; no permanent full-grid 80-byte table is allocated just to run ordinary ticks. A material batch cannot be split into partial entity updates because a buffer is full.
