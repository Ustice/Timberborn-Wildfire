...
The actual Yielder-only saver/loader fixture does **not** prove final whole-crop quantity after every native component loads. GatherableYieldGrower saves ripe progress1; its Load → FastForwardGrowth(1) → TimeTrigger.Finish can invoke Yielder.ResetYield. The actual native template factory puts the named Yielder before GatherableYieldGrower: a managed reproduction confirms saved3 becomes5 after ripe grower loading. The separate native PreInitialize correction rereads the same original native loader before model/reservation initialization; see [load correction evidence](native-gatherable-yield-load.md). Load-time YieldAdded must not be mistaken for a new gameplay generation.
