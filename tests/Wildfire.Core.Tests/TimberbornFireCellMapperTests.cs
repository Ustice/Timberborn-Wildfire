...
public sealed class TimberbornFireCellMapperTests
{
    ...
    public void BadwaterSourceActsWaterLikeWithoutBecomingSafeWater()
    {
        ...
        WildfireCompanionField companion = Assert.Single(mapper.CreateCompanionFields(grid, [badwater]));

        Assert.Equal(3, PackedCell.Water(mappedCell.PackedCell));
        Assert.Equal(WildfireMaterialClass.Badwater, companion.State.MaterialClass);
        Assert.Equal(WildfireContaminationBehavior.TaintedSource, companion.State.ContaminationBehavior);
    }
