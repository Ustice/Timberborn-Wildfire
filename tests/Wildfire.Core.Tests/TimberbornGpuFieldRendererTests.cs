...
public sealed class TimberbornGpuFieldRendererTests
{
    ...
    public void FireSimUsesDeterministicStochasticSmokeMovementAndDilution()
    {
        ...
        Assert.Contains("float directionalBias = lerp(0.08f, 3.5f, saturate((alignment + 1.0f) * 0.5f));", source, StringComparison.Ordinal);
        ...
        Assert.Contains("return DecayBy(movedSmoke, travelDecay);", source, StringComparison.Ordinal);
        Assert.Contains("uint retainedSmoke = smokeSource > 0u ? AtmosphericSmoke(oldAtmospheric) : DecayBy(AtmosphericSmoke(oldAtmospheric), 1u);", source, StringComparison.Ordinal);
        ...
        Assert.Contains("return SmokeSourceChoosesTarget(sourceCoordinate, targetCoordinate, sourceSmoke) ? movedSmoke : 0u;", source, StringComparison.Ordinal);
        ...
        Assert.Contains("float baseWeight = hasHorizontalDrift ? (diagonal ? 0.95f : 1.15f) : 0.28f;", source, StringComparison.Ordinal);
        Assert.Contains("float baseWeight = hasHorizontalDrift ? (diagonal ? 0.46f : 0.62f) : 0.18f;", source, StringComparison.Ordinal);
