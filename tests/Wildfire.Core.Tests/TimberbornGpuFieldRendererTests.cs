...
public sealed class TimberbornGpuFieldRendererTests
{
    ...
    public void SmokeAndSteamRenderersUseReleasePuffTuning()
    {
        ...
        Assert.Contains("private const int SmokePuffsPerCell  = 3;", indirectRenderer, StringComparison.Ordinal);
        Assert.Contains("(uint)(cellCount * SmokePuffsPerCell)", indirectRenderer, StringComparison.Ordinal);
        Assert.Contains("_smokeMaterial.SetFloat(\"_Radius\",        1.18f);", indirectRenderer, StringComparison.Ordinal);
        Assert.Contains("_smokeMaterial.SetFloat(\"_MaxOpacity\",    0.38f);", indirectRenderer, StringComparison.Ordinal);
        ...
        Assert.Contains("_steamMaterial.SetFloat(\"_MaxOpacity\",    0.34f);", indirectRenderer, StringComparison.Ordinal);
