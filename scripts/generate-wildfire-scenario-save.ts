...
type ScenarioProfileName = "persistence-matrix" | "stored-materials" | "world-consequence";
...
Options:
  ...
  --profile <name>               Scenario profile: world-consequence, stored-materials, persistence-matrix.
                                  Default: world-consequence.
...
const parseArgs = (args: string[]): Options => {
  ...
  args.reduce((_, arg, index) => {
    ...
    if (arg === "--allow-user-save-output") {
      ...
      options.profile = parseScenarioProfileName(requireValue(args, index + 1, arg));
      skipNext = true;
    } else if (arg.startsWith("--profile=")) {
      options.profile = parseScenarioProfileName(arg.slice("--profile=".length));
...
const parseScenarioProfileName = (value: string): ScenarioProfileName => {
  if (value === "world-consequence" || value === "stored-materials" || value === "persistence-matrix") {
    return value;
  }

  return fail(`Invalid --profile "${value}". Expected world-consequence, stored-materials, or persistence-matrix.`);
};
...
const worldConsequenceRequests = (): EntityRequest[] => [
...
const storedMaterialRequests = (): EntityRequest[] => [
  ...worldConsequenceRequests(),
  { category: "explosive-storage-pad", coordinate: { x: 40, y: 25, z: 2 }, template: "LargePile.Folktails" },
  { category: "contaminated-storage-pad", coordinate: { x: 43, y: 25, z: 2 }, template: "SmallTank.Folktails" },
  { category: "blast-witness-pad", coordinate: { x: 41, y: 27, z: 2 }, template: "Path" },
  { category: "contamination-witness-pad", coordinate: { x: 43, y: 27, z: 2 }, template: "Path" },
];

const persistenceMatrixRequests = (): EntityRequest[] => [
  ...storedMaterialRequests(),
  { category: "fertile-ash-gatherer-pad", coordinate: { x: 6, y: 37, z: 2 }, template: "Path" },
  { category: "tainted-washout-pad", coordinate: { x: 13, y: 44, z: 2 }, template: "BadwaterSource" },
  { category: "crop-counter-pad", coordinate: { x: 11, y: 5, z: 2 }, template: "Carrot" },
  { category: "clean-burn-duration-pad", coordinate: { x: 11, y: 41, z: 2 }, template: "Oak" },
];

const scenarioProfile = (profile: ScenarioProfileName): ScenarioProfile => {
  if (profile === "stored-materials") {
    return {
      description: "Fixture shape for TWF-176: stored-good burn, explosive stored goods, contaminated stored goods, blast witnesses, and contamination witnesses.",
      name: profile,
      requests: storedMaterialRequests(),
      validationTargets: [
        "Load this archive in Timberborn, then run qa-adjust-inventory stored-materials to stock live inventory targets with explosive and contaminated goods before burning storage.",
        "Run stored-good burn, native blast/heat-pulse, contaminated smoke/ash, Player.log scan, and final status evidence.",
        "If qa-adjust-inventory fails to find live inventory targets, the template needs storage-capable entities before QA can pass.",
      ],
    };
  }

  if (profile === "persistence-matrix") {
    return {
      description: "Fixture shape for TWF-081: stored goods, fertile ash collection, crop counters, tainted washout, and clean burn-duration targets.",
      name: profile,
      requests: persistenceMatrixRequests(),
      validationTargets: [
        "Load this archive in Timberborn and confirm all persistence matrix target classes exist before running save/quit/reload.",
        "Run before/after status around stored-goods loss, fertile ash collection/depletion, crop burn counters, tainted washout/no-resurrection, and burn-duration proof.",
        "If the manifest still reports inventory or terrain/channel blockers, add live setup or a dedicated save mutator before using it as accepted persistence evidence.",
      ],
    };
  }

  return {
    description: "General world-consequence scenario shape with trees, crops, storage-like structures, water, badwater, and camera lanes.",
    name: profile,
    requests: worldConsequenceRequests(),
    validationTargets: [
...
const createLayoutRequests = (profile: ScenarioProfileName = "world-consequence"): EntityRequest[] => scenarioProfile(profile).requests;
...
const sourceMaterialClassByCategory: Record<string, MaterialClass> = {
  ...
  "blast-witness-pad": "infrastructure",
  "central-camera-lane": "infrastructure",
  "clean-burn-duration-pad": "tree",
  "contaminated-storage-pad": "storage",
  "contamination-witness-pad": "infrastructure",
  "crop-pad": "crop",
  "crop-counter-pad": "crop",
  "explosive-storage-pad": "storage",
  "fertile-ash-gatherer-pad": "infrastructure",
  "mixed-material-structure-pad": "storage",
  "stored-water-pad": "storage",
  "tainted-washout-pad": "badwater",
  "tree-pad": "tree",
...
const cellMaterialClassFor = (request: EntityRequest): MaterialClass =>
  ["contaminated-storage-pad", "explosive-storage-pad", "mixed-material-structure-pad", "stored-water-pad"].includes(request.category)
    ? "building"
...
export const mutateWorldEntities = (
  world: JsonValue,
  profile: ScenarioProfileName = "world-consequence",
): { blockers: EntityPlacement[]; generated: EntityRequest[]; nextWorld: JsonValue } => {
  const nextWorld = cloneJson(world);
  const entities = asArray(asObject(nextWorld)?.Entities);
  if (!entities) {
    return {
      blockers: createLayoutRequests(profile).map((request) => ({ ...request, blockedReason: "world.json path Entities is not an array" })),
      generated: [],
      nextWorld,
    };
  }
  ...
  const planned = createLayoutRequests(profile);
...
const buildManifest = (
  ...
  const profile = scenarioProfile(options.profile);
  ...
  return {
    ...
    plan: {
      ...
      expectedEntityCountsByCategory: countByCategory(createLayoutRequests(options.profile)),
      layout: {
        baseHeight: 2,
    ...
    validationTargets: profile.validationTargets,
  };
};
...
const run = (): void => {
  ...
  const world = inspection.jsonFiles["world.json"];
  const mutation = options.mutateEntities && world ? mutateWorldEntities(world, options.profile) : { blockers: [], generated: [], nextWorld: world };
