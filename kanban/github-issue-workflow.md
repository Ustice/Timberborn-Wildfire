...
- Move to `status:waiting-for-dependency` when the issue is waiting on another GitHub issue or intra-ticket dependency that changes whether this issue can be implemented, reviewed, or accepted. The issue comment must link the dependency and name the reassessment trigger.
- Do not use `status:waiting-for-dependency` for coordination caution, shared-branch hygiene, avoiding parallel edits, or a preferred serial order. If the issue can make useful progress with careful scoping, keep it `status:ready`, record the sequencing caution in the issue comment or assignment packet, and name the branch/worktree constraint explicitly.
