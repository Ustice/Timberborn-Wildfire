...
- 2026-05-06 recovery worker reconciled the reviewed TWF-084 branch into the current dirty main checkout instead of replaying the old branch directly. Tree burn is now a separate consequence sink beside crop burn, fed from the shared `TimberbornBurnDamageService`, preserving current TWF-076 crop behavior plus newer storage, structure, explosive, and tunnel surfaces.
- 2026-05-06 recovery details: live tree registration now comes from Timberborn cuttable/tree targets in the adapter, with cuttable yield such as `Log` driving burn capacity. Default native mutation remains safe-unavailable, but status/QA telemetry exposes registered tree targets and nonzero tree consideration, burnable, and skipped-unsafe counters when a real tree target is hit. Verification passed: focused tree tests `7/7`, focused QA command bridge tests `86/86`, `dotnet build Wildfire.slnx --no-restore` with `0` warnings and `0` errors, and `git diff --check`. Keep in `04-verify` for fresh review before live QA.
- 2026-05-06 fresh review failed the recovered main implementation. Blocking finding: live target collection derives per-instance cuttable yield in `BuildResourceYieldTarget`, but `Collect` groups descriptors by `SpecId` and keeps only the first descriptor, so two live trees of the same spec with different remaining `Yielder` amounts can get the same burn capacity and yield-loss math. Move back to `03-in-progress`; preserve per-target yield/capacity through registration or use per-instance descriptor identity, and add a same-spec/different-yield deterministic test before fresh review.
- 2026-05-06 worker capacity fix: yield-backed burn-damage registrations now carry an optional per-registration descriptor override so same-spec tree targets preserve per-instance burn capacity. Live tree resource-yield registration passes the per-instance descriptor into registration, preventing two `Tree.Pine` targets with different remaining `Yielder` amounts from collapsing to the first descriptor's capacity.
- 2026-05-06 worker capacity-fix verification passed: focused tree tests `8/8`, `dotnet build Wildfire.slnx --no-restore` with `0` warnings and `0` errors, and `git diff --check`. Added a same-spec/different-yield regression where `1`-log and `3`-log `Tree.Pine` targets receive the same damage; only the `1`-log tree fully burns/kills while the `3`-log tree retains yield. Move back to `04-verify` for fresh review.
- 2026-05-06 fresh deterministic review passed with no blocking findings. Reviewer confirmed live tree registrations pass a per-registration `DescriptorOverride`, and `TimberbornBurnDamageService.BuildState` uses that override before the grouped descriptor catalog, so same-spec trees no longer collapse onto the first descriptor's capacity. Verification passed: `git diff --check` and focused tree tests `8/8`. Keep in `04-verify`; loaded-save tree/cuttable QA evidence is still required before integration.
- 2026-05-06 live QA passed under `~/Library/Application Support/Mechanistry/Timberborn/WildfireQA/TWF-084-tree-burn-live-20260506T203532Z` using `QA Tunnels and Booms/2026-05-06 16h18m, Day 2-5.autosave.timber`. Baseline registered real tree/cuttable targets with `burn_damage_registered_tree_burn_targets=1250` and `burn_damage_registered_tree_burn_owned_cells=3080`. `qa-delta-stimulus center-tree` hit a real tree at `x=25 y=24 z=6`, `target_index=16225`, `target_material=Tree`, `companion_target_id=973`.
- 2026-05-06 live QA proof: tree-burn consequence telemetry moved from immediate `considered=13`, `burnable=13`, to after 10 seconds `considered=13`, `burnable=13`, `skipped_unsafe_apis=25`, then after 20 seconds `considered=4`, `burnable=4`, `skipped_unsafe_apis=12`. Native tree yield/death/visual mutation remained precise safe-unavailable: `yield_lost=0`, `killed_trees=0`, `visual_state_updates=0`, with nonzero `tree_burn_skipped_unsafe_apis`; `Player.log` records `wildfire_timberborn_delta_consequence_sink_bound lane=tree_burn_consequences unavailable_api=true`. Move to `05-integration`.
- 2026-05-06 integration complete. Integration review passed with `git diff --check`, focused tree tests `8/8`, and focused QA command bridge tests `86/86`. Accepted scope is tree/cuttable target registration plus tree-burn consequence telemetry with precise safe-unavailable native mutation; future native tree yield/death/visual mutation still needs separate API proof.
