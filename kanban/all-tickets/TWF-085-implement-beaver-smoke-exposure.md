...
- Worker implementation adds normal-smoke accumulation inside the existing TWF-073 dispatcher: bounded dispatch batches, cooldown skips, smoke sample accumulation, reversible coughing safe-no-op state, decay/recovery after exposure clears, and candidate/skipped telemetry for choking and death.
- No native beaver health, incapacitation, pathing, work, or death API is invoked. Runtime continues to use the no-op actuator; unsafe choking/death outcomes are reported as skipped API telemetry only.
