...
- 2026-05-04/05 retry result: pass for the canonical source-count parity unit after `TWF-151`. Evidence root: `~/Library/Application Support/Mechanistry/Timberborn/WildfireQA/twf-141-parity-retry-20260505/`.
- Retry artifacts include `twf-141-diorama-50.fixture.json`, `twf-141-diorama-50.cells.bin`, `twf-141-diorama-50.cells.metadata.json`, `qa-readiness-live.txt`, and `importer-parity-comparison.md`.
- Source parity now matches exactly for the accepted 50x50 Diorama target: dimensions `50x50x23`; terrain `2503`; vegetation `103`; crop `0`; tree `1316`; building `179`; storage `23`; infrastructure `145`; water `32`; badwater `90`.
- Resolved-cell parity remains diagnostic-only and still mismatches because the snapshot exporter does not replay Timberborn's exact runtime placement and overwrite order. This does not block downstream tuning that consumes source importer parity, but it should stay visible if a future ticket needs cell-identity-perfect replay.
