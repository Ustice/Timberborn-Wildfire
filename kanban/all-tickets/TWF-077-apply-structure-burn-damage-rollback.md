...
- 2026-05-06 live QA: passed the owned structure proof on the `Fuel` save under `~/Library/Application Support/Mechanistry/Timberborn/WildfireQA/twf-077-115-127-128-129-live-20260506T134000Z/`. `building-owned-burn-damage-probe.txt` queued `target_selector=building` against `burn_damage_target_key=structure:-1621872456`, `burn_damage_spec_id=GathererFlag.Folktails(Clone)`, and `burn_damage_remaining_capacity=44`. `Player.log` then recorded `wildfire_timberborn_burn_damage_applied` and `wildfire_timberborn_structure_burn_damage_rollback_applied` with matched structure cells and positive shared burn-damage application. The final diff still needs fresh review before integration because the live target-registration classifier changed after the earlier review.
