...
- Reviewed deterministic implementation is parked on `~/repos/wildfire-TWF-070-steam-visual` / `codex/TWF-070-steam-visual` commit `07875a16`. It tunes clean field-driven steam to `puffs_per_cell=5`, `base_color=0.98,0.99,1.00`, `radius=0.92`, `height_offset=0.03`, `max_height=2.05`, `max_opacity=0.48`, `up_speed=2.35`, and `down_speed=1.20`, with a dedicated `wildfire_timberborn_gpu_indirect_renderer_steam_tuning` log token.
...
- Live visual QA is blocked: `bun scripts/invoke-timberborn-command.ts status --wait=5` wrote the command inbox but timed out waiting for `command-outbox.txt`, and the process list showed Steam without a loaded Timberborn process. No rendered steam readability is accepted until a high-resolution recording proves the tuned white steam distinct from smoke and fire.
