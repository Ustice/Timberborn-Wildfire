...
- 2026-05-20 coordination note: Implementation branch `codex/TWF-170-paused-ash` passed review and deterministic verification, with commit `d162f61f`. The fix preserves simulator-owned ash authority, refreshes material and transport buffer bindings during restore, and performs one GPU field renderer presentation pass during runtime initialization so paused saves can show ash before the first unpause dispatch. Checks passed in the worktree: `git diff --check`, targeted ash/persistence/presentation tests `41/41`, and full `dotnet test Wildfire.slnx --no-restore` `454/454`. Live QA is still blocked because `bun scripts/invoke-timberborn-command.ts status --wait=5` timed out waiting for `command-outbox.txt` and visible process listing did not include Timberborn. Keep this ticket out of integration until live QA loads an existing-ash save while paused, captures whole-scene screenshot/status proof, unpauses, and proves no duplicate ash entries or goods. After a successful QA pass, delete bulky videos/screenshots while preserving the lightweight manifest/log/status evidence.
