...
- 2026-05-04/05 final alignment made source count the canonical parity unit for this gate. Live import now reports vegetation, infrastructure, and filtered building/storage families separately; snapshot export reads installed Blueprints, counts only occupied `BlockObjectSpec.Blocks` entries instead of full `Size` bounding boxes, expands `OccupyAllBelow` water/badwater sources, treats tanks as storage, and skips inert saved objects that live import does not provide as sources.
- Latest artifact: `~/Library/Application Support/Mechanistry/Timberborn/WildfireQA/twf-151-family-split-20260504/importer-parity-comparison.md`. The 50x50 Diorama snapshot and live telemetry match dimensions and all source counts exactly: terrain `2503`, vegetation `103`, crop `0`, tree `1316`, building `179`, storage `23`, infrastructure `145`, water `32`, badwater `90`. Resolved-cell counts remain diagnostic-only because the snapshot exporter still does not replay Timberborn's exact runtime placement and overwrite order.
- A temporary live building-breakdown diagnostic was attempted on 2026-05-05 but the game locked on the loading screen before Wildfire initializer logs appeared. `Player.log` only showed early `gpath.c:115` assertions, so the diagnostic was removed and the accepted fix is based on Blueprint `Occupations != None`, deterministic exporter tests, and the existing successful live `qa-readiness` capture.
