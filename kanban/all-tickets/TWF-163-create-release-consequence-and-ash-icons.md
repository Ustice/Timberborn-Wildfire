...
- Create or bind a fertile ash good icon that mostly preserves the current dirt/ash look, but adds a small monochrome sprout emerging from the ash.
- Create or bind a tainted or contaminated ash icon that uses the same dirt/ash base image, with native monochrome contamination visual language.
...
- 2026-05-23 asset update: accepted root icons `good-fertile-ash.png` and `good-contaminated-ash.png`, copied them into `src/Wildfire.Timberborn/Data/Sprites/Goods/`, bound `FertileAsh` good/resource icons to `Sprites/Goods/FertileAshIcon`, and mirrored both icons into `docs/reference/assets/goods-icons/` plus the goods icon reference index and composite sheet.
- 2026-05-23 reference correction: the current integrated icon references live under `docs/reference/assets/`, not the removed `docs/reference/assets/wildfire-icons/` worktree artifact path. Use `docs/reference/assets/goods-icons/FertileAshIcon.png`, `docs/reference/assets/goods-icons/ContaminatedAshIcon.png`, `docs/reference/assets/goods-icons/composite.png`, `docs/reference/assets/menu-icons/composite.png`, and `docs/reference/assets/status-icons/composite.png` for the remaining binding and live UI screenshot pass.
- 2026-05-23 coordinator static check before rerunning live QA: the current main checkout has FertileAsh good and natural-resource icon bindings to `Sprites/Goods/FertileAshIcon`, plus `src/Wildfire.Timberborn/Data/Sprites/Goods/FertileAshIcon.png` and `ContaminatedAshIcon.png`. However, `TimberbornFertilizeToolButtons.cs` still creates both fertilize toolbar buttons with `BurnToolImageName = "DemolishResourcesTool"` and no Wildfire-owned fertilize-crop or fertilize-tree icon asset was found under `src/Wildfire.Timberborn`. Do not spend live QA on toolbar-icon acceptance until a worker either binds the intended fertilize toolbar sprites or documents that the current inherited tool image is the accepted blocker. The remaining live proof should then capture the good/resource icon and the fertilize crop/tree toolbar buttons in the real UI.
