...
- 2026-05-06 QA result: failed required live QA on `Fuel` evidence root `~/Library/Application Support/Mechanistry/Timberborn/WildfireQA/twf-144-148-live-20260506T071808Z/`. Positive behavior evidence exists: `slow-reactable` was active, the `50x50` Fuel save loaded, 25 imported center-tree changes were queued, final status showed `last_delta_count=50`, `started_burning=24/25`, real-field visual events, water-change proof, and later cooling back to `last_delta_count=0`. The gate still failed because the required normal gameplay recordings did not visibly show readable fire, smoke, ash, steam, or heat haze. Shader snapshot search did not find a committed accepted `slow-reactable` snapshot command or files. Smallest unblock: make the normal gameplay visual path plainly visible or provide a supported camera/renderer enablement path, then rerun this gate on Fuel or another stable real save.
