...
internal static partial class TimberbornOwnedMaterialCodec
{
    private static void WriteNativeDefinitions(BinaryWriter writer,OwnedNativeDefinitionSet definitions)=>
        ...
        {
            output.Write(value.EntityId.ToByteArray());WriteText(output,value.SpecId);output.Write((int)value.Shape);
            var profile=value.BodyProfile;
            WriteText(output,profile.SpecId);WriteText(output,profile.Type);output.Write(profile.FuelValue);
            output.Write(profile.DestructionThreshold);output.Write(profile.Flammability);
            output.Write(profile.Explosive);output.Write(profile.Contaminated);output.Write(profile.Known);
            WriteArray(output,value.LocalFootprint,(o,c)=>{o.Write(c.X);o.Write(c.Y);o.Write(c.Z);});
            WriteArray(output,value.Yields,(o,y)=>
            {
                o.Write((int)y.Role);WriteText(o,y.ComponentName);WriteText(o,y.GoodId);o.Write(y.Amount);o.Write(y.RemoveOnCut);
            });
            output.Write(value.BuildingCost is not null);
            if(value.BuildingCost is {} cost) WriteArray(output,cost,WriteStack);
        });
    private static OwnedNativeDefinitionSet ReadNativeDefinitions(BinaryReader reader,int maxOwners)=>new(
        ...
        {
            var id=new Guid(input.ReadBytes(16));string spec=ReadText(input);var shape=(TimberbornInitialBodyShape)input.ReadInt32();
