...
public enum OwnedNativeCompatibilityCapability { Unavailable, Complete }
...
public sealed record OwnedNamedYieldDefinition
{
    public OwnedNamedYieldDefinition(TimberbornCapturedYieldRole role, string componentName, string goodId, int amount, bool removeOnCut)
    {
        ...
        Role=role; ComponentName=componentName; GoodId=goodId; Amount=amount; RemoveOnCut=removeOnCut;
    }
...
public sealed class OwnedNativeDefinitionWitness
{
        ...
        IEnumerable<OwnedNamedYieldDefinition> yields,IEnumerable<TimberbornBurnDamageResourceStack>? buildingCost)
    {
        ...
        if(building != (buildingCost is not null)) throw new ArgumentException("Building witness requires its exact static cost, including empty cost.");
...
public sealed class OwnedNativeDefinitionSet
{
    ...
    public OwnedNativeDefinitionSet(IEnumerable<OwnedNativeDefinitionWitness> definitions)
    {
        ...
        if(values.Select(d=>d.EntityId).Distinct().Count()!=values.Length) throw new ArgumentException("Duplicate native witness.");
        Definitions=Array.AsReadOnly(values);_byId=values.ToDictionary(d=>d.EntityId);
    }
    ...
    internal void Validate(IReadOnlyList<OwnedConsequenceOwner> owners)
    {
        ...
        if(retained.Length!=Definitions.Count || retained.Any(o=>!_byId.TryGetValue(o.EntityId,out var d) ||
            d.SpecId!=o.Profile!.SpecId || d.Family!=o.Family)) throw new ArgumentException("Native witnesses must match exactly the retained owner identities.");
    }
