...
public sealed partial class TimberbornOwnedDeltaConsumer
{
    ...
    public static TimberbornOwnedDeltaConsumer CreateWithNativeDefinitions(TimberbornNativeMaterialRegistry origins,
        ...
        TimberbornResourceFuelCatalog? catalog = null) => guard.CaptureAtRest(() =>
            CreateWithNativeDefinitionsDuringCapture(origins, damage, effects, guard, bodies, catalog));

    // Caller owns this consumer's same guard through initial formation and final native reread.
    internal static TimberbornOwnedDeltaConsumer CreateWithNativeDefinitionsDuringCapture(TimberbornNativeMaterialRegistry origins,
        TimberbornBurnDamageService damage, TimberbornOwnedNativeEffects effects,
        Wildfire.Timberborn.Resources.INativeResourceMutationGuard guard, IReadOnlyList<TimberbornInitialMaterialBody> bodies,
        TimberbornResourceFuelCatalog? catalog = null)
    {
        ...
        if (owners.Length != damage.States.Count)
            throw new ArgumentException("Witnessed formation must account for every body.");
